...
- DNS(域名系统
    - >C/S
        - UDP ← 域名查询本身适用
            - <span style="color:lightgray">查到后建立连接,still need TCP</span>
            - FTP SMTP HTTP←TCP(对可靠要求高
    - 域名
        - 主机
            - 对应关系
                - >一对多/多对一
            ...
            - 次数
                ...
                - **时延**
                    - 点击连接→收到内容' t
                        - >↑暗示了建立连接+发送内容
                        - 最短
                            - H有IP映射记录: H与www.abc.com
                                - >不用DNS查询
                            - 1.5RTT+0.5RTT
                                - 建立,发送
                        - 最长
                            - get IP： H→本地) →根、顶、权限abc.com [3RTT
                            - 建立+发送：H与www.abc.com [2RTT
                            - >忽略H与本地时延(局域网
                                - ↑"H访问Internet上各sever RTT,忽略其他各种"
---
- FTP
    - >allow客户 指明 file 类型 与 格式
    - 带外传送
        - 控制info、数据info 通过不同的逻辑信道传送
    - 下载文件
        - 自顶而下(应用→..→物理
- --
- email
    ...
    - 协议
        - SMTP
            - >发push; TCP port 25
            - mail服务器之间or user→mail服务器
                - compare:
                    - Internet中, use HTTP
                        - HTTP也可收
                - <span style="color:lightgray"> (即visit 网页时不可用</span>
                    - PPP<span style="color:lightgray">(WAN protocol</span>
                    - UDP
                    - ARP<span style="color:lightgray">(不知主机MAC时</span>
            - format
                - 7bit ASCII码
    ...
    - element：
        ...
        - **HTTP**
            - **type**
                - ../1.0
                    - >非持续连接
                    - 每请求一个对象need 建立、关闭一次TCP连接
                        - eg.4个对象(1基本文本3gif图像)
                            - 4次link
                        - eg.含100个图片'html页面
                            - 1(基本html文件)+100(图片)=101
                -  --
                - ../1.1
                    - >持续link← 常默认
                        - connection中：
                    ...
                    - **流水线**
                        - 建立link: 1.5RTT
                            ...
                            - ←
                            - →
                        - HTTP请求报文&get HTTP基本文档: 1RTT~0.5RTT
                            - **→**（理想情况下，上一步可能捎带）
                                - ↑(从HTTP请求报文开始，从这start
                            - ←
                        - **引用**请求&send: 1RTT
