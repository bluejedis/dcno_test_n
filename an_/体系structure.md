...
好的，各位同学，我们开始第一章《计算机网络体系结构》的复习。这一章是整个计网的基础，概念多，但理解了就不难。我们的目标是：**最少的文字，最深的记忆，最快的解题**。跟紧我的思路，我们把这一章拿下！
...
* **形象记忆**:
    ...
    * **工作方式角度 (用户和快递员视角)**:
        ...
        * **核心部分**: 大量的路由器、交换机 (快递公司和中转站，提供服务)。
    * **功能组成角度 (功能模块视角)**:
        * **通信子网**: 负责送信的邮政系统，包括邮递员、运输车、分拣中心 (即物理层、链路层、网络层)。
        * **资源子网**: 信的内容和收发信的人 (即应用层、表示层、会话层)。
...
* **【计算题模板】: 交换时延计算**
    ...
    3.  **分组交换 (重点!)**:
        ...
        * **极简模板 (推荐)**: 总时延 = (源主机发送所有分组的时间) + (最后一个分组在链路上的传播时间)
            * $T_{总} = N \times \frac{P}{R} + (k-1) \times \frac{P}{R} + k \times d$
...
* **形象记忆**:
    * **速率 vs 带宽**:
        ...
        * **带宽 (Bandwidth)**: 你家水管设计的最大流水速度，即水管的粗细 (e.g., 100Mb/s)。
    * **吞吐量 (Throughput)**: 实际单位时间内流过的总水量，受限于最细的那段水管（瓶颈）。
    * **时延 (Delay)**: 一滴水从水厂到你家的时间。
        * **发送时延 (传输时延)**: 把一桶水(分组)全部倒进水管(链路)需要的时间。
        * **传播时延**: 水滴在水管里“飞”的时间。
        * **处理时延**: 水泵站(路由器)处理这桶水的时间。
        * **排队时延**: 前面还有别人的水在排队，你得等着。
